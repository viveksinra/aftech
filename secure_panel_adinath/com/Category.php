	...
	public function Resize( $tmp, $path ,$photo , $width,$height ){
	 ...
	 //update records
	  ...
	  if($action=='add'){//echo "insert into product(`photo`,`name`,`url`,`category`,`subcategory`) values ('$photo','$name','$url','$category','$subcategory')";
		 $insert=$db->fireQuery("insert into category(`photo`,`category`,`btn_name`,`detail`) values ('$photo','$category','$btn_name','$detail')");
	 if($insert) 
	  {
	  ...
	  {
	                     if($photo!=''){
                           $update=(trim($db->fireQuery("update `category` set `photo`='$photo',`category`='$category',`btn_name`='$btn_name',`detail`='$detail'  where id=".siteDecrypt($_REQUEST['id'])."")));
		    }else{
                         $update=(trim($db->fireQuery("update `category` set `photo`='$photo1',`category`='$category',`btn_name`='$btn_name',`detail`='$detail'  where id=".siteDecrypt($_REQUEST['id'])."")));
