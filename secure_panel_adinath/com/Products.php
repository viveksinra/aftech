<?php 
  class Products extends sqlConnection{
	 public $database;
	 public $page;
 ...
 public function checkActivate( $qry ){
	   $db = $this->database;
		  extract( $qry );
		  $rec_id=$_REQUEST['id'];	
	 	$qry = $db->fireQuery("update `products` set `status`='Active' where id = '$rec_id'");
		if($qry)
		{
...
public function checkDeactivate( $qry ){
	   $db = $this->database;
		  extract( $qry );
		  $rec_id=$_REQUEST['id'];	
	 	$qry = $db->fireQuery("update `products` set `status`='Inactive' where id = '$rec_id'");
	   ...
	   public function deletePhoto( $qry ){
	 	...
	 	$qry = $db->fireQuery("delete from `products` where id = '$rec_id'");
	  ...
	  foreach( $checkAll as $id )
	   {
		$qry = $db->fireQuery("delete from `products` where id = '$id'");
	...
	public function Resize( $tmp, $path ,$photo , $width,$height ){
	 ...
	 //update records
	  ...
	  if($action=='add'){
		 $insert=$db->fireQuery("insert into products(`category`,`photo`,`name`,`heading`,`detail`,`status`) values ('$category','$photo','$name','$heading','$detail','Active')");
	 if($insert) 
	  {
	  ...
	  {
	                     if($photo!=''){
                           $update=(trim($db->fireQuery("update `products` set `category`='$category',`photo`='$photo',`name`='$name',`heading`='$heading',`detail`='$detail',`status`='Active'  where id=".siteDecrypt($_REQUEST['id'])."")));
		    }else{
                         $update=(trim($db->fireQuery("update `products` set `category`='$category',`photo`='$photo1',`name`='$name',`heading`='$heading',`detail`='$detail',`status`='Active'  where id=".siteDecrypt($_REQUEST['id'])."")));
