...
public function checkDeactivate( $qry ){
	...
	public function Resize( $tmp, $path ,$photo , $width,$height ){
	 ...
	 //update records
	  ...
	  if($action=='add'){//echo "insert into user(`pan`,`name`,`phone`,`email`,`address`,`age`,`password`,`status`,`added_on`) values ('$photo','$name','$phone','$email','$address','$age','$password','Active',NOW())";
	 ...
	 if($insert) 
	  ...
	  {
	                     if($photo!=''){
                           $update=(trim($db->fireQuery("update `user` set `pan`='$photo',`name`='$name',`phone`='$phone',`email`='$email',`address`='$address',`age`='$age',`password`='$password',`status`='Active'  where id=".siteDecrypt($_REQUEST['id'])."")));
		    }else{
                         $update=(trim($db->fireQuery("update `user` set `pan`='$photo1',`name`='$name',`phone`='$phone',`email`='$email',`address`='$address',`age`='$age',`password`='$password',`status`='Active'  where id=".siteDecrypt($_REQUEST['id'])."")));
