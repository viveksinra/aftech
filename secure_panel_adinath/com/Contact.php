<?php 
 class Contact extends sqlConnection{
	 public $database;
	 public $page;
  ...
  public function checkActivate( $qry ){
	   $db = $this->database;
		  extract( $qry );
		  $rec_id=$_REQUEST['id'];	
	 	$qry = $db->fireQuery("update `contact` set `status`='Active' where id = '$rec_id'");
		
		if($qry)
		{
			$_SESSION['msg'] = "Record(s) Status Changed successfully!";
            header("location:contact-mgmt.php");die();
		}
		else
		{
...
public function checkDeactivate( $qry ){
	   $db = $this->database;
		  extract( $qry );
		  $rec_id=$_REQUEST['id'];	
	 	$qry = $db->fireQuery("update `contact` set `status`='Inactive' where id = '$rec_id'");
	   ...
	   public function deletePhoto( $qry ){
	 	...
	 	$qry = $db->fireQuery("delete from `contact` where id = '$rec_id'");
		if($qry)
		{
	  ...
	  foreach( $checkAll as $id )
	   {
		$qry = $db->fireQuery("delete from `contact` where id = '$id'");
		if($qry)
		{
			$_SESSION['msg'] = "Record(s) deleted successfully!";
		}
	...
	public function Resize( $tmp, $path ,$photo , $width,$height ){
	 ...
	 //update records
	  ...
	  if($action=='add'){
		 ...
		 $insert=$db->fireQuery("insert into contact(`photo`,`heading`) values ('$photo','$heading')");
	  ...
	  if($action=='edit')
	  {
		  if(in_array($ext,$valid_formats))
		  {
	  }else{     //$_SESSION['errormsg'] = "Invalid file format..";
				 // header("location:$this->page");die();
			  }
		 if($photo!=''){	  
		  $update=(trim($db->fireQuery("update `contact` set `photo`='$photo',`heading`='$heading' where id=".siteDecrypt($_REQUEST['id'])."")));
		 }else{
             $update=(trim($db->fireQuery("update `contact` set `photo`='$photo1',`heading`='$heading' where id=".siteDecrypt($_REQUEST['id'])."")));
		       }
	  if($update) 
	  {
