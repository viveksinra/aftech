<?php 
  class Partner extends sqlConnection{
	 public $database;
	 public $page;
 ...
 public function checkActivate( $qry ){
	   $db = $this->database;
		  extract( $qry );
		  $rec_id=$_REQUEST['id'];	
	 	$qry = $db->fireQuery("update `partner` set `status`='Active' where id = '$rec_id'");
		if($qry)
		{
			$_SESSION['msg'] = "Record(s) Status Changed successfully!";
            header("location:partner-mgmt.php");die();
		}
		else
		{
...
public function checkDeactivate( $qry ){
	   $db = $this->database;
		  extract( $qry );
		  $rec_id=$_REQUEST['id'];	
	 	$qry = $db->fireQuery("update `partner` set `status`='Inactive' where id = '$rec_id'");
	   ...
	   public function deletePhoto( $qry ){
	 	...
	 	$qry = $db->fireQuery("delete from `partner` where id = '$rec_id'");
	  ...
	  foreach( $checkAll as $id )
	   {
		$qry = $db->fireQuery("delete from `partner` where id = '$id'");
	...
	public function Resize( $tmp, $path ,$photo , $width,$height ){
	 ...
	 //update records
	   public function updatePhoto( $qry ){
		...
		$valid_formats = array("webp","WEBP","jpg", "png", "gif", "bmp","jpeg","JPEG","JPG","PNG","GIF","BMP");
		if(strlen($photo))
		{
	  ...
	  if($action=='add'){//echo "insert into product(`photo`,`name`,`url`,`category`,`subcategory`) values ('$photo','$name','$url','$category','$subcategory')";
		 $insert=$db->fireQuery("insert into partner(`photo`,`name`) values ('$photo','$name')");
	 if($insert) 
	  {
	  ...
	  {
	                     if($photo!=''){
                           $update=(trim($db->fireQuery("update `partner` set `photo`='$photo',`name`='$name'  where id=".siteDecrypt($_REQUEST['id'])."")));
		    }else{
                         $update=(trim($db->fireQuery("update `partner` set `photo`='$photo1',`name`='$name'  where id=".siteDecrypt($_REQUEST['id'])."")));
