<?php 
  class Advert extends sqlConnection{
	 public $database;
	 public $page;
 ...
 public function checkActivate( $qry ){
	   $db = $this->database;
		  extract( $qry );
		  $rec_id=$_REQUEST['id'];	
	 	$qry = $db->fireQuery("update `advert` set `status`='Active' where id = '$rec_id'");
		if($qry)
		{
			$_SESSION['msg'] = "Record(s) Status Changed successfully!";
            header("location:advert-mgmt.php");die();
		}
		else
		{
...
public function checkDeactivate( $qry ){
	   $db = $this->database;
		  extract( $qry );
		  $rec_id=$_REQUEST['id'];	
	 	$qry = $db->fireQuery("update `advert` set `status`='Inactive' where id = '$rec_id'");
	   ...
	   public function deletePhoto( $qry ){
	 	...
	 	$qry = $db->fireQuery("delete from `advert` where id = '$rec_id'");
	  ...
	  foreach( $checkAll as $id )
	   {
		$qry = $db->fireQuery("delete from `advert` where id = '$id'");
...
$status="Active";
	  if($action=='add'){
		 ...
		 $insert=$db->fireQuery("insert into advert(`photo`,`category`,`url`,`status`) values ('$photo','$category','$url','$status')");
	 if($insert) 
	  {
	  ...
	  {
	                     if($photo!=''){
                           $update=(trim($db->fireQuery("update `advert` set `photo`='$photo',`category`='$category',`url`='$url',`status`='$status' where id=".siteDecrypt($_REQUEST['id'])."")));
		    }else{
                         $update=(trim($db->fireQuery("update `advert` set `photo`='$photo1',`category`='$category',`url`='$url',`status`='$status' where id=".siteDecrypt($_REQUEST['id'])."")));
