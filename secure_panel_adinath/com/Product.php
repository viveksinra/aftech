	...
	public function Resize( $tmp, $path ,$photo , $width,$height ){
	 ...
	 //update records
	  ...
	  if($action=='add'){//echo "insert into product(`photo`,`name`,`url`,`category`,`subcategory`) values ('$photo','$name','$url','$category','$subcategory')";
		 $insert=$db->fireQuery("insert into product(`photo`,`name`,`url`,`category`,`subcategory`,`detail`) values ('$photo','$name','$url','$category','$subcategory','$detail')");
	 if($insert) 
	  {
	  ...
	  {
	                     if($photo!=''){
                           $update=(trim($db->fireQuery("update `product` set `photo`='$photo',`name`='$name',`url`='$url',`category`='$category',`subcategory`='$subcategory',`detail`='$detail'  where id=".siteDecrypt($_REQUEST['id'])."")));
		    }else{
                         $update=(trim($db->fireQuery("update `product` set `photo`='$photo1',`name`='$name',`url`='$url',`category`='$category',`subcategory`='$subcategory',`detail`='$detail'  where id=".siteDecrypt($_REQUEST['id'])."")));
