<?php 
  class Blog extends sqlConnection{
	 public $database;
	 public $page;
 ...
 public function checkActivate( $qry ){
	   $db = $this->database;
		  extract( $qry );
		  $rec_id=$_REQUEST['id'];	
	 	$qry = $db->fireQuery("update `blog` set `status`='Active' where id = '$rec_id'");
		if($qry)
		{
			$_SESSION['msg'] = "Record(s) Status Changed successfully!";
            header("location:blog-mgmt.php");die();
		}
		else
		{
...
public function checkDeactivate( $qry ){
	   $db = $this->database;
		  extract( $qry );
		  $rec_id=$_REQUEST['id'];	
	 	$qry = $db->fireQuery("update `blog` set `status`='Inactive' where id = '$rec_id'");
	   ...
	   public function deletePhoto( $qry ){
	 	...
	 	$qry = $db->fireQuery("delete from `blog` where id = '$rec_id'");
	  ...
	  foreach( $checkAll as $id )
	   {
		$qry = $db->fireQuery("delete from `blog` where id = '$id'");
	...
	public function Resize( $tmp, $path ,$photo , $width,$height ){
	 ...
	 //update records
	  ...
	  if($action=='add'){//echo "insert into product(`photo`,`name`,`url`,`category`,`subcategory`) values ('$photo','$name','$url','$category','$subcategory')";
		 $insert=$db->fireQuery("insert into blog(`photo`,`writtenby`,`heading`,`detail`,`date`) values ('$photo','$writtenby','$heading','$detail','$date')");
	 if($insert) 
	  {
	  ...
	  {
	                     if($photo!=''){
                           $update=(trim($db->fireQuery("update `blog` set `photo`='$photo',`writtenby`='$writtenby',`heading`='$heading',`detail`='$detail',`date`='$date'  where id=".siteDecrypt($_REQUEST['id'])."")));
		    }else{
                         $update=(trim($db->fireQuery("update `blog` set `photo`='$photo1',`writtenby`='$writtenby',`heading`='$heading',`detail`='$detail',`date`='$date'  where id=".siteDecrypt($_REQUEST['id'])."")));
