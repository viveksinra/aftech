	...
	public function Resize( $tmp, $path ,$photo , $width,$height ){
	 ...
	 //update records
	  ...
	  if($action=='add'){//echo "insert into product(`photo`,`name`,`url`,`category`,`subcategory`) values ('$photo','$name','$url','$category','$subcategory')";
		 $insert=$db->fireQuery("insert into subcategory(`photo`,`category`,`subcategory`) values ('$photo','$category','$subcategory')");
	 if($insert) 
	  {
	  ...
	  {
	                     if($photo!=''){
                           $update=(trim($db->fireQuery("update `subcategory` set `photo`='$photo',`category`='$category',`subcategory`='$subcategory'  where id=".siteDecrypt($_REQUEST['id'])."")));
		    }else{
                         $update=(trim($db->fireQuery("update `subcategory` set `photo`='$photo1',`category`='$category',`subcategory`='$subcategory'  where id=".siteDecrypt($_REQUEST['id'])."")));
