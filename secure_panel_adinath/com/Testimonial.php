<?php 
  class Testimonial extends sqlConnection{
	 public $database;
	 public $page;
 ...
 public function checkActivate( $qry ){
	   $db = $this->database;
		  extract( $qry );
		  $rec_id=$_REQUEST['id'];	
	 	$qry = $db->fireQuery("update `testimonial` set `status`='Active' where id = '$rec_id'");
		if($qry)
		{
			$_SESSION['msg'] = "Record(s) Status Changed successfully!";
            header("location:testimonial-mgmt.php");die();
		}
		else
		{
...
public function checkDeactivate( $qry ){
	   $db = $this->database;
		  extract( $qry );
		  $rec_id=$_REQUEST['id'];	
	 	$qry = $db->fireQuery("update `testimonial` set `status`='Inactive' where id = '$rec_id'");
	   ...
	   public function deletePhoto( $qry ){
	 	...
	 	$qry = $db->fireQuery("delete from `testimonial` where id = '$rec_id'");
	  ...
	  foreach( $checkAll as $id )
	   {
		$qry = $db->fireQuery("delete from `testimonial` where id = '$id'");
	...
	public function Resize( $tmp, $path ,$photo , $width,$height ){
	 ...
	 //update records
	  ...
	  if($action=='add'){
		 $insert=$db->fireQuery("insert into testimonial(`photo`,`name`,`post`,`star`,`message`) values ('$photo','$name','$post','$star','$msg')");
	 if($insert) 
	  {
	  ...
	  {
	                     if($photo!=''){
                           $update=(trim($db->fireQuery("update `testimonial` set `photo`='$photo',`name`='$name',`post`='$post',`star`='$star',`message`='$msg'  where id=".siteDecrypt($_REQUEST['id'])."")));
		    }else{
                         $update=(trim($db->fireQuery("update `testimonial` set `photo`='$photo1',`name`='$name',`post`='$post',`star`='$star',`message`='$msg'  where id=".siteDecrypt($_REQUEST['id'])."")));
