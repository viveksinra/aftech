<?php 
  class Test extends sqlConnection{
	 public $database;
	 public $page;
 ...
 public function checkActivate( $qry ){
	   $db = $this->database;
		...
		if($qry)
		{
            ...
            header("location:test-mgmt.php");die();
		}
		else
		{
...
public function checkDeactivate( $qry ){
	...
	public function Resize( $tmp, $path ,$photo , $width,$height ){
	 ...
	 //update records
	  ...
	  if($action=='add'){
		 $insert=(trim($db->fireQuery("insert into testimonial(`photo`,`name`,`msg`,`status`)
                     values ('$photo','$name','$detail','Active')")));
	 if($insert) 
	  {
	  ...
	  {
	                     if($photo!=''){
                           $update=(trim($db->fireQuery("update `testimonial` set `photo`='$photo',`name`='$name',`msg`='$detail',`status`='Active'  where id=".siteDecrypt($_REQUEST['id'])."")));
		    }else{
                         $update=(trim($db->fireQuery("update `testimonial` set `photo`='$photo1',`name`='$name',`mag`='$detail',`status`='Active'  where id=".siteDecrypt($_REQUEST['id'])."")));
